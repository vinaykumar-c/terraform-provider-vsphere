...
// guest_ip_addresses.
func buildAndSelectGuestIPs(d *schema.ResourceData, guest types.GuestInfo) ([]string, error) {
	log.Printf("[DEBUG] %s: Checking guest networking state", resourceVSphereVirtualMachineIDString(d))
	var v4primary, v4gw net.IP
	var v4net2addrs map[int32][]string
	...
	if len(addrs) < 1 && guest.IpAddress != "" {
	    ip1 := net.ParseIP(guest.IpAddress)
	    if ip1.To4() != nil {
	...
	if len(addrs) < 1 {
		...
		return addrs, d.Set("guest_ip_addresses", addrs)
	}
	var primary string
	switch {
	case v4primary != nil:
