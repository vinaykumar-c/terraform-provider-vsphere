...
func TestAccResourceVSphereVirtualMachine_clusterVMotion(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccResourceVSphereVirtualMachineConfigResourcePoolVMotion(fmt.Sprintf("%s/Resources", os.Getenv("TF_VAR_VSPHERE_ESXI_NESTED1"))),
				Check: resource.ComposeTestCheckFunc(
					testAccResourceVSphereVirtualMachineCheckExists(true),
					testAccResourceVSphereVirtualMachineCheckResourcePool(fmt.Sprintf("%s/Resources", os.Getenv("TF_VAR_VSPHERE_ESXI_NESTED1"))),
				),
			},
		},
	})
